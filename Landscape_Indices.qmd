...
Unify adjacent grassland patches: Helpful: https://github.com/rstudio/cheatsheets/blob/main/sf.pdf https://github.com/r-spatial/sf/issues/290
...
dflist <- lapply(seq(touch), function(i){
  v <- numeric(length(touch))
  v[touch[[i]]] <- i
  v
})

df <- as.data.frame(do.call(cbind, dflist)) # list vector is in column


while(any( unlist(listlength) != 0 )){
  ...
  dis <- lapply(seq(touchidlist), function(i){
    summarise(dlm_th_grass[touchidlist[[i]], ], 
            m=mean(AREA)) # this unifies the polygons
  })
  
  dis_col <- do.call(rbind, dis) # one object from the unified polys
  nrow(dis_col)
  
  touch <- st_touches(dis_col) # checking again for the loop 
  listlength <- lapply(touch, function(x) length(x))
